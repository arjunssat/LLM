...
if selected_item == 'Manual':
    if st.session_state.selected_image:
        outer_prompt = st.text_input("What is up?")
    else:
    ...
    if selected_topic:
        if st.session_state.selected_image is not None:
            ...
            init_prompt = st.selectbox(label=selected_topic,options=selected_list)
    ...
    if st.session_state.selected_image:
        ...
        if st.session_state.selected_image[0].name not in st.session_state.ai_generated_question.keys():
            ...
            init_prompt = st.selectbox(label="ai generated questions",options=questions)
            outer_prompt = init_prompt
